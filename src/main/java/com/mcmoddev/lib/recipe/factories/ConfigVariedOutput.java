...
public class ConfigVariedOutput implements IRecipeFactory {
	...
	public IRecipe parse(JsonContext context, JsonObject json) {
		...
		// load the data here, map the ingredients, setup the primer and return the
		// ShapedOreRecipe :)
		Map<Character, Ingredient> ingMap = Maps.newHashMap();

		JsonUtils.getJsonObject(json, "key").entrySet().stream()
				.filter(ent -> ent.getKey().length() == 1 && !ent.getKey().isEmpty()).forEach(ent -> ingMap
						.put(ent.getKey().toCharArray()[0], CraftingHelper.getIngredient(ent.getValue(), context)));
