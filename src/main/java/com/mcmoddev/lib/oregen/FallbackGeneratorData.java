...
public class FallbackGeneratorData {
	...
	public void addMaterial(@Nonnull final String materialName, @Nonnull final String blockName,
		...
		Map<MMDMaterial, List<String>> blockMap = materials.getOrDefault(targetDim,
	...
	public void setup() {
		for (Entry<Integer, Map<MMDMaterial, List<String>>> matMap : materials.entrySet()) {
			...
			for (Entry<MMDMaterial, List<String>> matList : mats.entrySet()) {
				...
				matList.getValue().stream().forEach(
						blockName -> spawnList.add(new WorldGenMinable(mat.getBlock(blockName).getDefaultState(),
